...
IfcAppliedValueSelect::~IfcAppliedValueSelect() {}
shared_ptr<IfcAppliedValueSelect> IfcAppliedValueSelect::readStepData( std::string& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		std::stringstream strs;
		strs << "unhandled inline argument: " << arg << " in function IFC4RC4::IfcAppliedValueSelect::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcAppliedValueSelect>();
}
