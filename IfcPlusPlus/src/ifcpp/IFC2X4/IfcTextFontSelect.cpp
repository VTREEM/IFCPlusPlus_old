...
IfcTextFontSelect::~IfcTextFontSelect() {}
shared_ptr<IfcTextFontSelect> IfcTextFontSelect::readStepData( std::string& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		std::stringstream strs;
		strs << "unhandled inline argument: " << arg << " in function IFC4RC4::IfcTextFontSelect::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcTextFontSelect>();
}
