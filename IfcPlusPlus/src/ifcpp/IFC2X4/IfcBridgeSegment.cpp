...
void IfcBridgeSegment::setEntity( shared_ptr<IfcPPEntity> other_entity )
{
...
void IfcBridgeSegment::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args<10 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcBridgeSegment, expecting 10, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcPPException( strserr.str().c_str() ); }
	#ifdef _DEBUG
...
void IfcBridgeSegment::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	...
	if( !ptr_self ) { throw IfcPPException( "IfcBridgeSegment::setInverseCounterparts: type mismatch" ); }
