...
void IfcBridge::setEntity( shared_ptr<IfcPPEntity> other_entity )
{
...
void IfcBridge::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args<11 ){ std::stringstream strserr; strserr << "Wrong parameter count for entity IfcBridge, expecting 11, having " << num_args << ". Object id: " << getId() << std::endl; throw IfcPPException( strserr.str().c_str() ); }
	#ifdef _DEBUG
...
void IfcBridge::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
