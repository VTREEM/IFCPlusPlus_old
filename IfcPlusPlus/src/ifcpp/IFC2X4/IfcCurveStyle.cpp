...
void IfcCurveStyle::readStepData( std::vector<std::string>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_CurveFont = IfcCurveFontOrScaledCurveFontSelect::readStepData( args[2], map );
	m_CurveWidth = IfcSizeSelect::readStepData( args[3], map );
	m_CurveColour = IfcColour::readStepData( args[4], map );
}
