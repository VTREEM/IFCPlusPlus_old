...
IfcStyleAssignmentSelect::~IfcStyleAssignmentSelect() {}
shared_ptr<IfcStyleAssignmentSelect> IfcStyleAssignmentSelect::readStepData( std::string& arg, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	// Read SELECT TYPE
	...
	else
	{
		...
		std::stringstream strs;
		strs << "unhandled inline argument: " << arg << " in function IFC4RC4::IfcStyleAssignmentSelect::readStepData" << std::endl;
		throw IfcPPException( strs.str() );
	}
	return shared_ptr<IfcStyleAssignmentSelect>();
}
